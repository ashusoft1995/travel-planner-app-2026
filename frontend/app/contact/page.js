...
/** Default contact routing: primary admin; matches API `adminTarget`. */
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await submitContactMessage({
        ...
        otherAdminDetail:
          form.adminTarget === "Other" ? form.otherAdminDetail.trim() : "",
      });
      const recipient =
        form.adminTarget === "Other"
          ? form.otherAdminDetail.trim()
          : form.adminTarget;
      ...
      <div className="container max-w-6xl">
            ...
            <div className="card-surface p-8">
              ...
              <div className="mt-6 rounded-2xl border border-[var(--border)] bg-black/[0.02] p-5 dark:bg-white/5">
                ...
                <p className="mt-4 flex items-center gap-2 text-sm text-[var(--text)]">
                  ...
                  <a href="tel:+251997255611" className="font-semibold hover:underline">
                    {PRIMARY_ADMIN.phoneIntl}
                  </a>
                ...
                <p className="mt-2 text-sm text-[var(--text)]">
                  Telegram:{" "}
                  <a
                    href={PRIMARY_ADMIN.telegramUrl}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <div className="mt-6">
                ...
                <ul className="mt-3 space-y-3 text-sm">
                  {OTHER_ADMINS.map((a) => (
                    <li
                      key={a.name}
                      className="flex items-center justify-between gap-2 rounded-xl border border-[var(--border)] px-4 py-3"
                    >
              ...
              <ul className="mt-4 space-y-4 text-sm">
                <li className="flex gap-3">
                  ...
                  <a
                    ...
                    className="font-medium text-[var(--text)] hover:text-brand-600 hover:underline dark:hover:text-accent-yellow"
                  >
                    {OFFICE.email}
                  </a>
                </li>
                <li className="flex gap-3">
                  ...
                  <a
                    ...
                    className="font-semibold text-[var(--text)] hover:text-brand-600 hover:underline dark:hover:text-accent-yellow"
                  >
                    {OFFICE.phoneDisplay}
                  </a>
                ...
                <li className="flex gap-3">
                  ...
                  <a
                    ...
                    className="font-semibold text-brand-600 hover:underline dark:text-accent-yellow"
                  >
                    Instagram — {SOCIAL.instagramLabel}
                  </a>
                </li>
                <li className="flex gap-3">
          ...
          >
            <div>
              ...
              <p className="mt-2 text-sm text-[var(--muted)]">
                Default recipient:{" "}
                <strong className="text-[var(--text)]">{PRIMARY_ADMIN.name}</strong> (
                {PRIMARY_ADMIN.handle}). Use the dropdown to message another admin or
                choose Other and say who should receive it.
              </p>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    adminTarget: e.target.value,
                    otherAdminDetail:
                      e.target.value === "Other" ? p.otherAdminDetail : "",
                  }))
                }
              >
            ...
            {form.adminTarget === "Other" && (
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[var(--muted)]">
                  Which admin or team? (required)
                </label>
                <input
                  className="form-field w-full"
                  placeholder="e.g. name, role, or department"
                  value={form.otherAdminDetail}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, otherAdminDetail: e.target.value }))
                  }
                  required
                />
              </div>
            ...
            <textarea
              rows={5}
              className="form-field"
              placeholder="Your message to admin support…"
              value={form.message}
        ...
        {user && (
          ...
          >
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">
              Threads where your account email matches the message. Admin replies appear here and
            ...
            {myMessages.length === 0 ? (
              ...
              <ul className="mt-6 space-y-4">
                {myMessages.map((m) => (
                  <li
                    key={m.id}
                    className="rounded-2xl border border-[var(--border)] bg-black/[0.02] p-4 dark:bg-white/5"
                  >
                    <p className="text-xs text-[var(--muted)]">
                      {m.createdAt ? new Date(m.createdAt).toLocaleString() : ""}
                    </p>
                    <p className="mt-1 font-semibold text-[var(--text)]">
                      {m.subject || "Message to admin"}
                    </p>
                    <p className="mt-2 text-sm text-[var(--text)]">{m.message}</p>
                    {Array.isArray(m.replies) && m.replies.length > 0 && (
                      <div className="mt-3 border-t border-[var(--border)] pt-3">
                        <p className="text-xs font-bold uppercase text-brand-600 dark:text-accent-yellow">
                          Admin replies
                        </p>
                        <ul className="mt-2 space-y-2">
                          {m.replies.map((r) => (
                            <li
                              key={r.id}
                              className="rounded-lg bg-brand-50/80 p-3 text-sm dark:bg-brand-950/30"
                            >
